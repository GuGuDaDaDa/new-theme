  ...
  async function load(restoring = false) {
    ...
    try {
      ...
      if (!response.ok || internalPath(response.url, base) !== target)
        throw new Error('加载失败，请重试或直接访问下一页。');
      ...
      status.textContent = `已新增 ${added.length} 篇文章${next ? '' : '，已显示全部文章'}。`;
